...
for cond_key, title, lang, task in cond_configs:
    ...
    bars = ax1.bar(range(6), values, color='#4C72B0')
    ...
    bars = ax2.bar(range(6), values, color='#C44E52')
    ...
    ax3.set_title('Dimension Correlation', fontweight='bold')
    for i in range(5):
        for j in range(6):
            val = corr_data[i, j]
            if not np.isnan(val):
    ...
    bars = ax4.bar(range(6), values, color='#8172B2')
    ax4.set_xlabel('Model')
    ax4.set_ylabel('Jaccard')
    ax4.set_title('Jaccard', fontweight='bold')
    ax4.set_xticks(range(6))
    ax4.set_xticklabels(model_names, rotation=45, ha='right', fontsize=7)
    ...
    plt.suptitle(title, fontsize=13, fontweight='bold', y=1.05)
