...
其中 $y_i$ 为人类基准值，$\hat{y}_i$ 为模型预测值，$n$ 为维度数（本研究中 $n=5$）。越低越好，反映模型能否准确预测动词对应的五维参数值。

**RSA（表征相似性分析）**：

$$RSA(X, Y) = Spearman(vec(D_X), vec(D_Y))$$

其中 $D_X$ 和 $D_Y$ 分别为模型和人类的动词距离矩阵，$vec(\cdot)$ 表示将矩阵上三角向量化。范围[-1, 1]，越高越好，反映模型对动词之间相似性关系的理解是否与人类一致。

**CKA（中心核对齐）**：

$$CKA(X, Y) = \frac{HSIC(K_X, K_Y)}{\sqrt{HSIC(K_X, X_K) \cdot HSIC(K_Y, K_Y)}}$$

其中 $K_X$ 和 $K_Y$ 分别为模型和人类的核矩阵（本研究使用线性核 $K = X X^T$），$HSIC$ 为希尔伯特-施密特独立性准则。范围[0, 1]，越高越好，反映模型的动词表征空间结构与人类的相似程度。

**Jaccard（维度选择一致性）**：

$$J(A, B) = \frac{|A \cap B|}{|A \cup B|}$$

其中 $A$ 为人类认为显著区分的维度集合，$B$ 为模型认为显著区分的维度集合。范围[0, 1]，越高越好，反映模型是否使用了与人类相同的关键维度来区分动词。
