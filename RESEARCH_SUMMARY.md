...
**MSE（均方误差）**：衡量模型预测参数与人类基准参数之间的差异，越低越好。在本研究中，MSE反映模型能否准确预测动词对应的五维参数值。

**RSA（表征相似性分析）**：衡量模型与人类的动词距离结构是否一致，范围[-1, 1]，越高越好。在本研究中，RSA反映模型对动词之间相似性关系的理解是否与人类一致。

**CKA（中心核对齐）**：衡量模型与人类的表征空间对齐程度，范围[0, 1]，越高越好。在本研究中，CKA反映模型的动词表征空间结构与人类的相似程度。

**Jaccard（维度选择一致性）**：衡量模型与人类在区分动词时选择的维度是否一致，范围[0, 1]，越高越好。在本研究中，Jaccard反映模型是否使用了与人类相同的关键维度来区分动词。
